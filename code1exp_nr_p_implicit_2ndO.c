...
long double phi_s[3][n*n*n],f_s[3][n*n*n],f_dc[n*n*n],slip_s[3][n*n*n],
	    LAPslip[n*n*n],LAPf[n*n*n],tmpslip2[n*n*n],tmpslip1[n*n*n],crLAPphi[n*n*n],LAPphi[n*n*n];
...
long double mesh2particle(struct particle *,int,long double *);
...
long double mesh2particle(struct particle *pp,int p_id,long double *meshf)
{
...
void particle2mesh(struct particle * pp,int p_id,long double *meshphi,long double ap)
{

	int i,j,k;
	int anchor[3];
	long double rvphi=0.0,del[8],deld;
	long double gamma,vmgsqr,acube = ap*ap*ap;
...
{
   return(Vamp*exp(-fff*lbd));
}


long double V_f(long double fff)
{
   return(-Vamp*lbd*exp(-fff*lbd));
...
{
   return(Vamp*lbd*lbd*exp(-fff*lbd));
...
{ 
   ...
   a_t = ai;
    ...
    omdmbini= 0.99999999*(cpmc)*pow((a0/ai),3.0)/(cpmc*a0*a0*a0/(ai*ai*ai) + (1.0-cpmc));
  ...
  fb_a =0.0/a_t;
	...
	if(j%jprint==0)
	fprintf(fpback,"%Lf\t%.10Lf\t%.10Lf\t%.10Lf\t%.10Lf\t%.10Lf\t%.10Lf\n",
...
//####################################################################################################
	...
	printf("\n	Min k_mag is %Lf per MPc:: corr lmbda is %.10Lf MPc",1.0/(dx[0]*lenfac*((long double) n)),dx[0]*lenfac*((long double) n));
	printf("\n	Max k_mag is %Lf per MPc:: corr lmbda is %.10Lf Mpc",sqrt(maxkmagsqr)/lenfac,lenfac/sqrt(maxkmagsqr));
	printf("\n	kbins is %d\n",kbins);

	printf("\nLengthscales:");
	printf("\n	Grid Length is %.5lf MPc",dx[0]*lenfac*((long double) n));
	printf("\n	dx is %.10Lf MPc\n",dx[0]*lenfac);
...
void slip_fft_cal()
{    
	 long double kfac,kfac2,tmp,tmptmp,Vvl,V_fvl,m[3];
		
	 int i,l1,l2,r1,r2,j,j2,mm;
	...
	for(i=0;i<tN;++i)
	{
	 ...
	 // particle2mesh(tmpp,i,tmpphi,ak);
		...
		m[0] = (16.0*phi[l1]+16.0*phi[r1]); 
		m[1] = (-phi[l2]-phi[r2]);
		m[2] = m[0] + m[1] -30.0*phi[i];
		LAPphi[i]+= (m[2]/d2[j]);
	...
	//	LAPf[i] += (-scf_rhs[l2][0]+16.0*scf_rhs[l1][0]-30.0*scf_rhs[i][0]+16.0*scf_rhs[r1][0]-scf_rhs[r2][0])/(n3sqrt*d2[j]); 
		...
		slip_rhs[i][0]-= (3.0*(phi_s[0][i]*phi_s[1][i]+phi_s[1][i]*phi_s[2][i]+phi_s[2][i]*phi_s[0][i]));
		slip_rhs[i][0]+= (phi_s[0][i]*(phi_s[1][i]-slip_s[1][i])+phi_s[1][i]*(phi_s[2][i]-slip_s[2][i])+phi_s[2][i]*(phi_s[0][i]-slip_s[0][i]));
		slip_rhs[i][0]+= (phi_s[1][i]*(phi_s[0][i]-slip_s[0][i])+phi_s[2][i]*(phi_s[1][i]-slip_s[1][i])+phi_s[0][i]*(phi_s[2][i]-slip_s[2][i]));
		slip_rhs[i][0]-= ((phi_s[0][i]-slip_s[0][i])*(phi_s[1][i]-slip_s[1][i])+(phi_s[1][i]-slip_s[1][i])*(phi_s[2][i]-slip_s[2][i])
					+(phi_s[2][i]-slip_s[2][i])*(phi_s[0][i]-slip_s[0][i]));
		slip_rhs[i][0]-= 4.0*phi[i]*crLAPphi[i];
	...
	#pragma omp parallel for private(j,l1,l2,r1,r2,Vvl,V_fvl,m,j2)
	for(i=0;i<tN;++i)
	{
		...
		m[0] = (-8.0*slip_rhs[l1][0]+8.0*slip_rhs[r1][0]); 
		m[1] = (slip_rhs[l2][0]-slip_rhs[r2][0]);
		m[2] = m[0] + m[1] ;
		
		
		slip_s[j][i] = m[2]/(n3sqrt*d1[j]);
		...
		l1 = i + ((n+ind_grid[i][j2]-1)%n)*((int)(pow(n,2-j2))) - ind_grid[i][j2]*((int)(pow(n,2-j2)));
		l2 = i + ((n+ind_grid[i][j2]-2)%n)*((int)(pow(n,2-j2))) - ind_grid[i][j2]*((int)(pow(n,2-j2)));
		r1 = i + ((n+ind_grid[i][j2]+1)%n)*((int)(pow(n,2-j2))) - ind_grid[i][j2]*((int)(pow(n,2-j2)));
		r2 = i + ((n+ind_grid[i][j2]+2)%n)*((int)(pow(n,2-j2))) - ind_grid[i][j2]*((int)(pow(n,2-j2)));

		m[0] = (-8.0*phi_s[j][l1]+8.0*phi_s[j][r1]); 
		m[1] = (phi_s[j][l2]-phi_s[j][r2]);
		m[2] = m[0] + m[1] ;
	  ...
	  for(ci=0;ci<tN;++ci)
	   ...
	   // LAPslip[ci] = 0.0;
		...
		fl = ( V_fvl/(a_t*a_t) + 3.0*f_a[ci]/a - 3.0*f_a[ci]*phi_a[ci] - 6.0*(phi[ci]-slip[ci])*f_a[ci]/a 
			...
			+ 4.0*(LAPf[ci]/a)*(phi[ci]-slip[ci])*phi[ci]/(a*a_t*a_t)  ; 

		/*fl+= (((-12.0*f_a[ci]*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])/a + 6.0*f_a[ci]*phi_a[ci]*phi[ci] - 6.0*(phi[ci]-slip[ci])*f_a[ci]*phi_a[ci]
			    - 4.0*(phi[ci]-slip[ci])*(phi_a[ci]-slip_a[ci])*f_a[ci])/(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])))
			    /(a_t*a_t));

		fl+= (2.0*slip[ci]*
			   (f_s[0][ci]*(phi_s[0][ci]-slip_s[0][ci])+f_s[1][ci]*(phi_s[1][ci]-slip_s[1][ci])+f_s[2][ci]*(phi_s[2][ci]-slip_s[2][ci]))
				/(a*a_t*a*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));

		fl+= (4.0*phi[ci]*phi[ci]*(LAPf[ci]/(a*a))/(a_t*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));
...
int evolve(long double aini, long double astp)
{
    ...
    long double phiacc1[n*n*n],phiacc2[n*n*n],facc1[n*n*n],facc2[n*n*n],pacc1[n*n*n][3],pacc2[n*n*n][3],v,gamma,phiavg,phi_aavg,phi_savg[3],slip_savg[3],fsg,kfac2;
    long double vmagsqr;
    int anchor[3];
...
/////////////////////phi acceleration calculation Step 1/////////////////////////////////////////////////////////////////////////////////
		...
		phiacc1[ci] = ( (a_t*a_t/(a*a) + 2.0*a_tt/a )*(slip[ci]-phi[ci])/(a_t*a_t) + (slip_a[ci]-4.0*phi_a[ci])/a + (1.0/3.0)*LAPslip[ci]/(a*a*a_t*a_t)
				-(tuldss[ci])/(6.0*Mpl*Mpl*a_t*a_t) ) / (1.0+2.0*slip[ci])  - (a_tt*phi_a[ci]/(a_t*a_t)) ;	
		phiacc1[ci]+= ((2.0*(slip[ci]-phi[ci])*(slip[ci]-phi[ci])*(1.0+2.0*a_tt*a)/(a*a) - 6.0*phi_a[ci]*slip[ci]/a - phi_a[ci]*phi_a[ci])
							/(1.0+2.0*slip[ci]));
		phiacc1[ci]+= ((4.0*(phi[ci]-slip[ci])*(phi_a[ci]-slip_a[ci])/a + phi_a[ci]*(phi_a[ci]-slip_a[ci])*(a_t/a)*(a_t/a))/(1.0+2.0*slip[ci]));
		
		phiacc1[ci]+= ((0.5*(phi_s[0][ci]*phi_s[0][ci]+phi_s[1][ci]*phi_s[1][ci]+phi_s[2][ci]*phi_s[2][ci])/(a*a) +
			      (phi_s[0][ci]*(phi_s[0][ci]-slip_s[0][ci])+phi_s[1][ci]*(phi_s[1][ci]-slip_s[1][ci])+phi_s[2][ci]*(phi_s[2][ci]-slip_s[2][ci])
				+ (phi_s[0][ci]-slip_s[0][ci])*(phi_s[0][ci]-slip_s[0][ci])+(phi_s[1][ci]-slip_s[1][ci])*(phi_s[1][ci]-slip_s[1][ci])
			        + (phi_s[2][ci]-slip_s[2][ci])*(phi_s[2][ci]-slip_s[2][ci]) 
			      )/(3.0*a*a)
				)/(a_t*a_t*(1.0+2.0*slip[ci])));
		phiacc1[ci]+= ((2.0*phi[ci]*(LAPphi[ci]+LAPslip[ci]) + 2.0*(phi[ci]-slip[ci])*(LAPphi[ci]-LAPslip[ci]))/(a*a_t*a_t*a*(1.0+2.0*slip[ci])));
		...
		facc1[ci] = ( (V_fvl/(a_t*a_t) + 3.0*f_a[ci]/a - 3.0*f_a[ci]*phi_a[ci] - 6.0*(phi[ci]-slip[ci])*f_a[ci]/a 
				- (phi_a[ci]-slip_a[ci])*f_a[ci])/(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))

				+(f_s[0][ci]*slip_s[0][ci]+f_s[1][ci]*slip_s[1][ci]+f_s[2][ci]*slip_s[2][ci])/
				  (a*a*a_t*a_t*(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))) 

				-(LAPf[ci]/a)*((1.0+2.0*phi[ci])/(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])))/(a*a_t*a_t) )
			        -a_tt*f_a[ci]/(a_t*a_t); 

/*
		facc1[ci]+= (((-12.0*f_a[ci]*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])/a + 6.0*f_a[ci]*phi_a[ci]*phi[ci] - 6.0*(phi[ci]-slip[ci])*f_a[ci]*phi_a[ci]
			    - 4.0*(phi[ci]-slip[ci])*(phi_a[ci]-slip_a[ci])*f_a[ci])/(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])))
			    /(a_t*a_t));

		facc1[ci]+= (2.0*slip[ci]*
			   (f_s[0][ci]*(phi_s[0][ci]-slip_s[0][ci])+f_s[1][ci]*(phi_s[1][ci]-slip_s[1][ci])+f_s[2][ci]*(phi_s[2][ci]-slip_s[2][ci]))
				/(a*a_t*a*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));

		facc1[ci]+= (4.0*phi[ci]*phi[ci]*(LAPf[ci]/(a*a))/(a_t*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));
...
/////////////////////phi  acceleration calculation Final/////////////////////////////////////////////////////////////////////////////////
		...
		phiacc2[ci] = ( (a_t*a_t/(ak*ak) + 2.0*a_tt/ak )*(slip[ci]-phi[ci])/(a_t*a_t) + (slip_a[ci]-4.0*tmpphi_a[ci])/ak +
				...
				-(tuldss[ci])/(6.0*Mpl*Mpl*a_t*a_t) ) / (1.0+2.0*slip[ci])  - (a_tt*tmpphi_a[ci]/(a_t*a_t)) ;	
		phiacc2[ci]+= ((2.0*(slip[ci]-phi[ci])*(slip[ci]-phi[ci])*(1.0+2.0*a_tt*ak)/(ak*ak) - 6.0*tmpphi_a[ci]*slip[ci]/ak - tmpphi_a[ci]*tmpphi_a[ci])
							/(1.0+2.0*slip[ci]));
		phiacc2[ci]+= ((4.0*(phi[ci]-slip[ci])*(tmpphi_a[ci]-slip_a[ci])/ak
				 + tmpphi_a[ci]*(tmpphi_a[ci]-slip_a[ci])*(a_t/ak)*(a_t/ak))/(1.0+2.0*slip[ci]));
		
		phiacc2[ci]+= ((0.5*(phi_s[0][ci]*phi_s[0][ci]+phi_s[1][ci]*phi_s[1][ci]+phi_s[2][ci]*phi_s[2][ci])/(ak*ak) +
			      (phi_s[0][ci]*(phi_s[0][ci]-slip_s[0][ci])+phi_s[1][ci]*(phi_s[1][ci]-slip_s[1][ci])+phi_s[2][ci]*(phi_s[2][ci]-slip_s[2][ci])
				+ (phi_s[0][ci]-slip_s[0][ci])*(phi_s[0][ci]-slip_s[0][ci])+(phi_s[1][ci]-slip_s[1][ci])*(phi_s[1][ci]-slip_s[1][ci])
			        + (phi_s[2][ci]-slip_s[2][ci])*(phi_s[2][ci]-slip_s[2][ci]) 
			      )/(3.0*ak*ak)
				)/(a_t*a_t*(1.0+2.0*slip[ci])));
		phiacc2[ci]+= ((2.0*phi[ci]*(LAPphi[ci]+LAPslip[ci]) + 2.0*(phi[ci]-slip[ci])*(LAPphi[ci]-LAPslip[ci]))/(ak*a_t*a_t*ak*(1.0+2.0*slip[ci])));
		...
		facc2[ci] = ( (V_fvl/(a_t*a_t) + 3.0*tmpf_a[ci]/ak - 3.0*tmpf_a[ci]*tmpphi_a[ci] - 6.0*(phi[ci]-slip[ci])*tmpf_a[ci]/ak 
				- (tmpphi_a[ci]-slip_a[ci])*tmpf_a[ci])/(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))

				+(f_s[0][ci]*slip_s[0][ci]+f_s[1][ci]*slip_s[1][ci]+f_s[2][ci]*slip_s[2][ci])/
				  (ak*ak*a_t*a_t*(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))) 

				-(LAPf[ci]/ak)*((1.0+2.0*phi[ci])/(-1.0+2.0*(phi[ci]-slip[ci])-4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])))/(ak*a_t*a_t) )
			        -a_tt*tmpf_a[ci]/(a_t*a_t); 


		/*facc2[ci]+= (((-12.0*tmpf_a[ci]*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])/ak + 6.0*tmpf_a[ci]*tmpphi_a[ci]*phi[ci] 
				- 6.0*(phi[ci]-slip[ci])*tmpf_a[ci]*tmpphi_a[ci]
			    - 4.0*(phi[ci]-slip[ci])*(tmpphi_a[ci]-slip_a[ci])*tmpf_a[ci])/(1.0-2.0*(phi[ci]-slip[ci])
				+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci])))
			    /(a_t*a_t));

		facc2[ci]+= (2.0*slip[ci]*
			   (f_s[0][ci]*(phi_s[0][ci]-slip_s[0][ci])+f_s[1][ci]*(phi_s[1][ci]-slip_s[1][ci])+f_s[2][ci]*(phi_s[2][ci]-slip_s[2][ci]))
				/(ak*a_t*ak*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));

		facc2[ci]+= (4.0*phi[ci]*phi[ci]*(LAPf[ci]/(ak*ak))/(a_t*a_t*(1.0-2.0*(phi[ci]-slip[ci])+4.0*(phi[ci]-slip[ci])*(phi[ci]-slip[ci]))));
		*/
		...
		*/
		...
		f_a[ci] = f_a[ci]+da*0.5*(facc2[ci]+facc1[ci]);

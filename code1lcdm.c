...
double *phi, *psi, *f,*phi_a, *psi_a, *f_a,*tul00,*tuldss;
double phi_s[3][n*n*n],psi_s[3][n*n*n],f_s[3][n*n*n],LAPphi[n*n*n],LAPpsi[n*n*n],LAPf[n*n*n],usty[n*n*n],psty[n*n*n];
double *tmpphi, *tmppsi, *tmpf,*tmpphi_a, *tmppsi_a, *tmpf_a, *ini_vel0,*ini_vel1,*ini_vel2,m=1.0;
double LAPmet_slip[n*n*n];
double dx[3];
...
double  omdmb, omdeb, a, ak, a_t, a_tt, Vamp, ai, a0, da, ak, fbt, fb_at,a_zels;
	...
	for(i=0;i<=kbins;++i)
	{
		...
		if(kbincnt[i]!=0)
	        fprintf(fspwrite,"%lf\t%lf\t%.20lf\t%.20lf\t%.20lf\n",
					a/ai,i*dk,pwspctrm[i]/(kbincnt[i]),pwspctrm[i]*ai*ai/(kbincnt[i]*a*a),W_cic[i]);
...
void particle2mesh(struct particle * pp,int p_id,double *meshpsi,double *meshphi,double ap)
{
	...
	for(i=0;i<8;++i)
	{
		...
		tul00[k]+= del[i]*gamma*(1.0+3.0*rvphi-rvpsi-gamma*gamma*(vmgsqr*ap*ap*rvphi+rvpsi))/(ap*ap*ap);
		//tuldss[k]+= (vmgsqr*gamma/3.0)*del[i]*(1.0+3.0*rvphi-rvpsi-gamma*gamma*(vmgsqr*ap*ap*rvphi+rvpsi))/(ap*ap*ap);
		tuldss[k]+= (vmgsqr*gamma/3.0)*del[i]*(3.0*rvphi-rvphi-gamma*gamma*(vmgsqr*ap*ap*rvphi+rvphi))/(ap*ap*ap);
		psty[k]+= sqrt(vmgsqr)+m*del[i]*gamma*(1.0+3.0*rvphi-gamma*gamma*vmgsqr*ap*ap*rvphi)/(ap*ap*ap);
		usty[k]+= del[i]*gamma*(-1.0-gamma*gamma)/(6.0*a_t*a_t*Mpl*Mpl*ap);
...
void initialise()
{
	...
	#pragma omp parallel for
	  for(ci=0;ci<tN;++ci)
	  {
		...
		psi_s[j][ci] = (psi[l2]-8.0*psi[l1]+8.0*psi[r1]-psi[r2])/(12.0*dx[j]); 
...
int evolve(double aini, double astp)
{
    ...
    double phiacc,facc,pacc[3],v,gamma,phiavg,psiavg,phi_aavg,psi_aavg,phi_savg[3],psi_savg[3],fsg,psiold;
    double vmagsqr;
    int anchor[3];
    double lplphi,lplf;
    int l2,l1,r1,r2;
	 ...
	 for(ci=0;ci<tN;++ci)
	  {
		...
		for(i=0;i<3;++i)
		{
			...
			pacc[i] = (p[ci].v[i]*a_t*a_t*a_t*vmagsqr*(-2.0*a*a_t*(phiavg+psiavg)-a*a*phi_aavg*a_t+a*a_t)
				 +p[ci].v[i]*a_t*(fsg + psi_aavg*a_t -2.0*a_t/a + 2.0*phi_aavg*a_t -phi_savg[i])
				 -psi_savg[i]/(a*a))/(a_t*a_t) - a_tt*p[ci].v[i]/(a_t*a_t);
			fprintf(fptest1,"%.20lf\t%.20lf\t%.20lf\t%.20lf\n",
				p[ci].v[i]*a_t*a_t*a_t*vmagsqr*(-2.0*a*a_t*(phiavg+psiavg)-a*a*phi_aavg*a_t+a*a_t),
					p[ci].v[i]*a_t*(fsg + psi_aavg*a_t + 2.0*phi_aavg*a_t -phi_savg[i]),psi_savg[i]/(a*a),p[ci].v[i]*a_t);
...
/////////////////////phi acceleration calculation Step 1/////////////////////////////////////////////////////////////////////////////////
		...
		phiacc = (1.0/(a_t*a*a_t*a))*(- 2.0*a*psi[ci]*a_tt + (1.0/3.0)*(LAPmet_slip[ci])
					      -a*a*tuldss[ci]/(6.0*Mpl*Mpl))  -psi[ci]/(a*a) 
						- 3.0*phi_a[ci]/a -psi_a[ci]/a - a_tt*phi_a[ci]/(a_t*a_t);
...
/////////////////////Intermediate Tul calculations and Psi construction//////////////////////////////////////////
	...
	#pragma omp parallel for
	  for(ci=0;ci<tN;++ci)
	  {
		tmppsi[ci] = tmpphi[ci];//(1.0/usty[ci])*( LAPphi[ci]/(3.0*a_t*a_t) -  
					//		(ak*ak)*psty[ci]/(Mpl*Mpl*6.0*a_t*a_t) +0.5);
		tmppsi_a[ci] =tmpphi_a[ci] ;// 2.0*(tmppsi[ci] - psi[ci])/da;
	...
	#pragma omp parallel for
	  for(ci=0;ci<tN;++ci)
	  {
		...
		psi_s[j][ci] = phi_s[j][ci];// (tmppsi[l2]-8.0*tmppsi[l1]+8.0*tmppsi[r1]-tmppsi[r2])/(12.0*dx[j]); 
		
		//LAPpsi[ci] = LAPphi[ci] ;//+= (-tmppsi[l2]+16.0*tmppsi[l1]-30.0*tmppsi[ci]+16.0*tmppsi[r1]-tmppsi[r2])/(12.0*dx[j]*dx[j]); 

		LAPmet_slip[ci] +=  (-(tmpphi[l2]-tmppsi[l2])/dx[j]+16.0*(tmpphi[l1]-tmppsi[l1])/dx[j]
				-30.0*(tmpphi[ci]-tmppsi[ci])/dx[j]+16.0*(tmpphi[r1]-tmppsi[r1])/dx[j]-(tmpphi[r2]-tmppsi[r2])/dx[j])/(12.0*dx[j]); 
	 ...
	 for(ci=0;ci<tN;++ci)
	  {
		...
		for(i=0;i<3;++i)
		{
			...
			pacc[i] = (tmpp[ci].v[i]*a_t*a_t*a_t*vmagsqr*(-2.0*ak*a_t*(phiavg+psiavg)-ak*ak*phi_aavg*a_t+ak*a_t)
				 +tmpp[ci].v[i]*a_t*(fsg + psi_aavg*a_t -2.0*a_t/ak + 2.0*phi_aavg*a_t -phi_savg[i])
				 -psi_savg[i]/(ak*ak))/(a_t*a_t) - a_tt*tmpp[ci].v[i]/(a_t*a_t);
		...
		*/
			p[ci].x[i] = p[ci].x[i] + 0.5*da*tmpp[ci].v[i];
...
/////////////////////phi  acceleration calculation Final/////////////////////////////////////////////////////////////////////////////////
		...
		phiacc = (1.0/(a_t*ak*a_t*ak))*(- 2.0*ak*tmppsi[ci]*a_tt + (1.0/3.0)*(LAPmet_slip[ci])
					      -ak*ak*tuldss[ci]/(6.0*Mpl*Mpl))  -tmppsi[ci]/(ak*ak) 
						- 3.0*tmpphi_a[ci]/ak -tmppsi_a[ci]/ak - a_tt*tmpphi_a[ci]/(a_t*a_t);
...
///////////////////////////////////////Final Tul and Psi recosntruction/////////////////////////////////////////////////////////////
	...
	#pragma omp parallel for
	  for(ci=0;ci<tN;++ci)
	  {
		psi[ci] = phi[ci];//(1.0/usty[ci])*( LAPphi[ci]/(3.0*a_t*a_t)  - 
				//			(ak*ak)*psty[ci]/(Mpl*Mpl*6.0*a_t*a_t) +0.5);
		psi_a[ci] = phi_a[ci] ;//2.0*(psi[ci]-tmppsi[ci] )/da;
	...
	#pragma omp parallel for
	  for(ci=0;ci<tN;++ci)
	  {
		...
		psi_s[j][ci] = phi_s[j][ci] ;// (psi[l2]-8.0*psi[l1]+8.0*psi[r1]-psi[r2])/(12.0*dx[j]); 
		
		//LAPpsi[ci] =LAPphi[ci] ;//+= (-psi[l2]+16.0*psi[l1]-30.0*psi[ci]+16.0*psi[r1]-psi[r2])/(12.0*dx[j]*dx[j]); 

		LAPmet_slip[ci] += (-(phi[l2]-psi[l2])/dx[j]+16.0*(phi[l1]-psi[l1])/dx[j]
				-30.0*(phi[ci]-psi[ci])/dx[j]+16.0*(phi[r1]-psi[r1])/dx[j]-(phi[r2]-psi[r2])/dx[j])/(12.0*dx[j]); 
